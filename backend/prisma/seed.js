...
function getMinRelicLevel(phase) {
    return Math.min(phase + 2, 9);
}
function getPhaseTP(phase) {
    return phase * 15000;
}
function getPhaseUnlockRequirements(phase) {
    if (phase === 1)
        return null;
...
function getPlanetDifficulty(phase) {
    if (phase <= 2)
        return 'easy';
    if (phase <= 4)
        return 'medium';
...
function getPlanetTP(phase) {
    return phase * 5000;
}
function getPlanetStrategyTips(name) {
    const tips = {
...
function getMissionDifficulty(phase, mission) {
...
function getMissionTP(phase, type) {
    const base = phase * 1500;
    return type === 'special' ? base * 1.5 : base;
}
function getEnemyDescription(planet, mission) {
    const enemies = [
...
function getWaveBreakdown(planet, mission) {
    return [
        {
            waveNumber: 1,
...
function getSpecialWaveBreakdown(planet) {
    return [
        {
            waveNumber: 1,
...
async function createSampleSquads(contributorId, adminId, missions) {
    const squads = [];
    ...
    const cloneTroopersSquad = await prisma.squad.create({
        data: {
            ...
            squadType: client_1.SquadType.CLONE_TROOPERS,
...
Gear Priority: Rex (Leader) > Fives > Echo > Arc Trooper > Clone Sergeant`,
    ...
    const jediSquad = await prisma.squad.create({
        data: {
            ...
            squadType: client_1.SquadType.JEDI,
...
Gear Priority: Yoda > Mace Windu > Obi-Wan > Ahsoka > Kanan`,
    ...
    const separatistSquad = await prisma.squad.create({
        data: {
            ...
            squadType: client_1.SquadType.SEPARATISTS,
    ...
    const additionalSquads = [
        {
            ...
            squadType: client_1.SquadType.GALACTIC_REPUBLIC,
        ...
        },
        {
            ...
            squadType: client_1.SquadType.MANDALORIANS,
        ...
        },
        {
            ...
            squadType: client_1.SquadType.SITH,
...
async function createMissionRecommendations(squads, missions) {
